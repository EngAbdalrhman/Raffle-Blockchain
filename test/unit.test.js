...
!developmentChains.includes(network.name) // run if we are in local chain
  ? describe.skip
      ...
      describe("fulfillRandomWords", function () {
        ...
        it("picks a winner, resets, and sends money", async () => {
          ...
          await new Promise(async (resolve, reject) => {
            ...
            try {
              const tx = await raffle.performUpkeep("0x"); // same as []
              const txReceipt = await tx.wait(1);
              startingBalance = await accounts[2].getBalance();
              await vrfCoordinatorV2Mock.fulfillRandomWords(
                txReceipt.events[1].args.requestId,
                raffle.address
              ); // why to call it from mock and it already called in contract by performup
            } catch (e) {
              reject(e);
            }
          });
